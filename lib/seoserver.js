...
(function() {
  var $, SeoServer, args, express, logentries, memcached, querystring, seoserver, _,
  ...
  SeoServer = (function() {
    ...
    SeoServer.prototype.buildURL = function(request) {
      var params;
      params = _(request.query).pick(this.defaultConfig.getParamWhitelist);
      if (_(params).isEmpty()) {
    ...
    SeoServer.prototype.logResponseStats = function(request, headers, time) {
      ...
      url = this.buildURL(request).replace(new RegExp("" + this.defaultConfig.host), '');
